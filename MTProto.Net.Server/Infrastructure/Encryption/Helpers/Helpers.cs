...
namespace MTProto.NET.Server.Infrastructure.Encryption.Helpers.Legacy
{
	public class Helpers
	{
		...
		public static AESKeyData CalcKey_2(byte[] sharedKey, byte[] msgKey, bool client)
		{
			var x = client ? 0 : 8;
			var sha2aText = new byte[52];
			var sha2bText = new byte[52];
			Array.Copy(msgKey, sha2aText, msgKey.Length);
			Array.Copy(sharedKey, x, sha2aText, 16, 36);
			var sha2a = SHA256.Create().ComputeHash(sha2aText);
			Array.Copy(sharedKey, 40 + x, sha2bText, 0, 36);
			Array.Copy(msgKey, 0, sha2bText, 36, msgKey.Length);
			var sha2b = SHA256.Create().ComputeHash(sha2bText);
			var key = new byte[32];
			var iv = new byte[32];
			Array.Copy(sha2a, 0, key, 0, 8);
			Array.Copy(sha2b, 8, key, 8, 16);
			Array.Copy(sha2a, 24, key, 24,  8);

			Array.Copy(sha2b, 0, iv, 0, 8);
			Array.Copy(sha2a, 8, iv, 8, 16);
			Array.Copy(sha2b, 24, iv, 24, 8);
